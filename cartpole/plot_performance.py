...
def extract_iter_avg_ret(file_path):
    ...
    if outer_iter_match and final_avg_match:
        it = int(outer_iter_match.group(1))
        entry = by_iter.setdefault(it, {})
        entry["avg"] = float(final_avg_match.group(1))
        if final_sem_match:
...
def plot_mean_sem(x, y, sem, label, color=None):
    line, = plt.plot(x, y, label=label, linewidth=LINE_WIDTH, color=color)
...
iters1, avg_rets1, sem_rets1 = extract_iter_avg_ret(file1)
iters2, avg_rets2, sem_rets2 = extract_iter_avg_ret(file2)
