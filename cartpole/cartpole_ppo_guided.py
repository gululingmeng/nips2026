...
class PPOBuffer(object):
    ...
    def make_batch(self, gamma_rl, gae_lambda, device):
        ...
        actions = torch.tensor(np.asarray(self.actions, dtype=np.int64), dtype=torch.long, device=device)
...
# ============================================================
# PPO actor-critic for discrete CartPole actions
# ============================================================
...
class PPOAgent(object):
    ...
    def select_action(self,
        ...
        return a, action_info, noisy_state
...
def rollout_episode(env,
    ...
    while not done:
        a, action_info, noisy_state = agent.select_action(
            s,
        ...
        if store_to_buffer:
            agent.store_transition(s, action_info, float(r_aug), ns, float(done))
...
def train_pac_guided_ppo(env_train,
    ...
    for k in range(pac_cfg.max_outer_iters):
        ...
        if p_lb >= pac_cfg.p_min and cert_safe_rate == 1:
...
def main():
    ...
    env_train = CartPoleSafetyEnv(env_name="CartPole-v1", max_steps=500, seed=13)
    ...
    # Optionally add noise during PPO data collection.
