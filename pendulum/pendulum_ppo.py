    ...
    for k in range(pac_cfg.max_outer_iters):
        ...
        if (k % pac_cfg.test_every) == 0 or (k == pac_cfg.max_outer_iters - 1):
            ...
            if eval_stats["safe_rate"] == 1.0:
                ...
                print("[stop] iter={} safe_eval={:.3f} == 1.000, epsilon={:.4f}".format(
                    k, eval_stats["safe_rate"], eps))
                final_result = {
                    "outer_iter": k,
...
def main():
    ...
    print("\n==== FINAL RESULT ====")
    print("Stopped by safe_eval? {}".format(res["stopped_by_safe_eval"]))
