...
class TD3Config:
    ...
    batch_size: int = 128
    exploration_std: float = 0.20
    exploration_clip: float = 0.50
...
class TD3Agent(object):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            next_a = torch.clamp(next_a, -self.max_action, self.max_action)
...
def train_pac_guided_td3(env_train, env_eval, agent, pac_cfg, cert_nn_cfg, device,
...
def main():
    ...
    agent = TD3Agent(env_train.state_dim, env_train.action_dim, env_train.max_action, td3_cfg, device)
