...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, hidden=256, log_std_min=-20.0, log_std_max=2.0):
        super().__init__()
    ...
    def sample(self, x, deterministic=False):
        ...
        log_prob = normal.log_prob(z) - torch.log(self.max_action * (1.0 - y.pow(2)) + 1e-6)
...
def train_pac_guided_sac(env_train, env_eval, agent, pac_cfg, cert_nn_cfg, device,
...
def main():
    ...
    agent = SACAgent(env_train.state_dim, env_train.action_dim, env_train.max_action, sac_cfg, device)
