...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, action_low, action_high, hidden=256, log_std_init=-0.5):
        super().__init__()
    ...
    def sample(self, x, deterministic=False):
        ...
        correction = torch.log(self.action_scale * (1.0 - y.pow(2)) + 1e-6)
    ...
    def evaluate_actions(self, x, actions):
        ...
        std = torch.exp(log_std)
        normal = torch.distributions.Normal(mean, std)
        scaled = torch.clamp((actions - self.action_mid) / self.action_scale, -0.999999, 0.999999)
        z = self.atanh(scaled)
        y = torch.tanh(z)
        correction = torch.log(self.action_scale * (1.0 - y.pow(2)) + 1e-6)
        log_prob = (normal.log_prob(z) - correction).sum(dim=-1)
...
class PPOAgent(object):
    def __init__(self, state_dim, action_dim, action_low, action_high, cfg, device):
        ...
        self.actor = Actor(state_dim, action_dim, self.action_low, self.action_high,
                           log_std_init=cfg.log_std_init).to(device)
...
def train_pac_guided_ppo(env_train,
                          env_eval,
                          agent,
                          pac_cfg,
                          cert_nn_cfg,
