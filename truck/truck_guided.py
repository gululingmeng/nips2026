...
class DDPGAgent(object):
    def __init__(self, state_dim, action_dim, action_low, action_high, cfg, device):
        ...
        self.action_scale = float(np.max(np.abs(self.action_high)))

        self.actor = Actor(state_dim, action_dim, self.action_scale).to(device)
        self.actor_targ = Actor(state_dim, action_dim, self.action_scale).to(device)
    ...
    def select_action(self,
        ...
        with torch.no_grad():
            action = self.actor(s).squeeze(0).cpu().numpy().astype(np.float32)
