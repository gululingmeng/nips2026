...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, action_low, action_high, hidden=256):
        ...
        low = self.action_low_np.reshape(1, action_dim)
        high = self.action_high_np.reshape(1, action_dim)
    ...
    def forward(self, x):
        action = self.action_mid + self.action_scale * self.net(x)
...
class TD3Agent(object):
    def __init__(self, state_dim, action_dim, action_low, action_high, cfg, device):
        ...
        self.actor = Actor(state_dim, action_dim, self.action_low, self.action_high).to(device)
        self.actor_targ = Actor(state_dim, action_dim, self.action_low, self.action_high).to(device)
    ...
    def select_action(self, state, deterministic=False, input_noise_cfg=None, rng=None):
        ...
        if not deterministic:
            if rng is None:
                rng = np.random.default_rng()
            noise = rng.normal(0.0, self.cfg.exploration_std, size=action.shape).astype(np.float32)
    ...
    def update(self):
        ...
        low = torch.tensor(self.action_low, dtype=torch.float32, device=self.device).view(1, -1)
        high = torch.tensor(self.action_high, dtype=torch.float32, device=self.device).view(1, -1)

        with torch.no_grad():
            noise = torch.randn_like(batch["action"]) * self.cfg.policy_noise
            noise = torch.clamp(noise, -self.cfg.noise_clip, self.cfg.noise_clip)
            next_a = self.actor_targ(batch["next_state"]) + noise
...
def solve_sampled_certificate_nn(cert_rollouts,
    ...
    if len(transitions) == 0 or X_pre.shape[0] == 0:
        return CertificateResult(
            feasible=False, model_state=None, c_bc=1.0, gamma_bc=gamma_bc,
            ...
            status="insufficient-pre-or-transitions",
            pre_key=pre_key, post_key=post_key,
        )
    ...
    X_s_t = torch.tensor(np.asarray([s for s, _ in transitions], dtype=np.float32),
                         dtype=torch.float32, device=device)
    X_ns_t = torch.tensor(np.asarray([ns for _, ns in transitions], dtype=np.float32),
                          dtype=torch.float32, device=device)
    ...
    for epoch in range(nn_cfg.epochs):
        ...
        if X_pre_t is not None and X_pre_t.shape[0] > 0:
            ...
            loss_pre = torch.tensor(0.0, dtype=torch.float32, device=device)

        if X_post_t is not None and X_post_t.shape[0] > 0:
            eta_post = model(X_post_t)
            loss_post = F.relu(1.0 - eta_post).mean()
        else:
            loss_post = torch.tensor(0.0, dtype=torch.float32, device=device)
    ...
    if constraints_satisfied:
        ...
        if X_post.shape[0] == 0:
            status = "constraints-satisfied-post-empty"
        else:
        ...
        p_lb = 0.0
        if X_post.shape[0] == 0:
            status = "epoch-limit-infeasible-post-empty"
        else:
...
def train_pac_guided_td3(env_train,
                          env_eval,
                          agent,
                          pac_cfg,
                          cert_nn_cfg,
        ...
        if p_lb >= pac_cfg.p_min:
            ...
            print(
                ...
                "iter_time={:.2f}s total_time={:.2f}s statusA={} statusB={} status={}".format(
                    ...
                    cert_res.cert_A.status, cert_res.cert_B.status, cert_res.status
                )
            )
...
def main():
    ...
    env_train = TruckSafetyEnv(dt=0.4, max_steps=100, seed=7, switch_prob=0.005, process_noise_std=0.0)
