...
# ============================================================
# SAC actor-critic for discrete 10-room actions
# ============================================================
...
class SACAgent(object):
    ...
    def __init__(self, state_dim, action_dim, cfg, device):
        ...
        self.critic1_targ = QNetwork(state_dim, action_dim).to(device)
        self.critic1_targ.load_state_dict(self.critic1.state_dict())
        self.critic2 = QNetwork(state_dim, action_dim).to(device)
        self.critic2_targ = QNetwork(state_dim, action_dim).to(device)
    ...
    def select_action(self, state, deterministic=False, input_noise_cfg=None, rng=None):
        ...
        if deterministic:
            ...
            if rng is None:
                rng = np.random.default_rng()
            a = int(rng.choice(self.action_dim, p=probs))
        return a, a, noisy_state

    def store_transition(self, s, action_info, r, ns, d):
        self.replay.add(s, int(action_info), r, ns, d)

    def store_random_transition(self, s, a, r, ns, d):
        self.replay.add(s, int(a), r, ns, d)
    ...
    def update(self):
        ...
        with torch.no_grad():
            next_logits = self.actor(batch["next_state"])
            next_probs = F.softmax(next_logits, dim=-1)
            next_log_probs = F.log_softmax(next_logits, dim=-1)
            next_q1 = self.critic1_targ(batch["next_state"])
            next_q2 = self.critic2_targ(batch["next_state"])
            next_q = torch.min(next_q1, next_q2)
            next_v = (next_probs * (next_q - self.cfg.alpha * next_log_probs)).sum(dim=-1, keepdim=True)
            y = batch["reward"] + self.cfg.gamma_rl * (1.0 - batch["done"]) * next_v

        q1 = self.critic1(batch["state"]).gather(1, batch["action"])
        q2 = self.critic2(batch["state"]).gather(1, batch["action"])
        ...
        q1_pi = self.critic1(batch["state"])
        q2_pi = self.critic2(batch["state"])
        q_pi = torch.min(q1_pi, q2_pi)
        actor_loss = (probs * (self.cfg.alpha * log_probs - q_pi)).sum(dim=-1).mean()
...
def train_sac_baseline(env_train: TenRoomBenchmarkEnv,
...
def main() -> None:
    ...
    history, res = train_sac_baseline(
        env_train=env_train,
        env_eval=env_eval,
        agent=agent,
        train_cfg=train_cfg,
